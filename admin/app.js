...
const state = {
  ...
  entries: [],
  admins: [],
  auth: { signedIn: false, email: null, isAdmin: false },
  saveTimer: null,
};
...
async function onGoogleSignIn() {
  ...
  btn.disabled = true;
  $('#err-auth').textContent = '';
  try {
    // Redirects away to Google and comes back to this same URL.
    await state.db.signInWithGoogle();
  } catch (err) {
    console.error(err);
    $('#err-auth').textContent =
      'google sign-in isn’t set up yet — enable it in supabase → authentication → providers';
    btn.disabled = false;
  }
}
...
async function onSignOut() {
  ...
  state.auth = { signedIn: false, email: null, isAdmin: false };
...
/**
 * Single place that decides which screen you land on. Called on boot and again
 * whenever the auth state changes (sign in, sign out, token refresh).
 */
async function routeByAuth({ landOn = 'screen-queue' } = {}) {
  state.auth = await state.db.getAuth();
  ...
  if (!state.auth.isAdmin) {
    $('#denied-email').textContent = state.auth.email || '';
    show('screen-denied');
    return;
  }
...
async function loadAdmins() {
  try {
    state.admins = await state.db.listAdminEmails();
  } catch (err) {
...
function renderAdmins() {
  ...
  const onlyOne = state.admins.length <= 1;

  $('#admin-rows').innerHTML = state.admins
    .map((email) => {
      const isMe = email.toLowerCase() === me;
      // Removing yourself, or the last admin, would lock everyone out. The
      // database refuses the last-admin case too — this just keeps it off screen.
      const locked = isMe || onlyOne;
      return `<div class="admin-row" data-email="${esc(email)}">
          <span class="email">${esc(email)}</span>
          ${isMe ? '<span class="you-tag">you</span>' : ''}
          ${
            locked
              ? ''
              : `<button class="mini-btn danger" data-act="remove-admin" type="button"
                   title="remove ${esc(email)}"
                   aria-label="remove ${esc(email)}">${icons.close}</button>`
          }
        </div>`;
    })
    .join('');
...
async function onAddAdmin(e) {
  e.preventDefault();
  const input = $('#in-admin-email');
  const email = input.value.trim().toLowerCase();
  $('#err-admin').textContent = '';
  input.classList.remove('is-error');

  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    $('#err-admin').textContent = 'enter a valid email address';
    input.classList.add('is-error');
    return;
  }

  if (state.admins.some((a) => a.toLowerCase() === email)) {
    $('#err-admin').textContent = 'that address is already an admin';
    input.classList.add('is-error');
    return;
  }

  try {
    await state.db.addAdminEmail(email);
    input.value = '';
    await loadAdmins();
    toast(`${email} can now sign in`);
  } catch (err) {
    console.error(err);
    $('#err-admin').textContent = "couldn't add that address";
  }
}

async function onAdminRowClick(e) {
  const btn = e.target.closest('[data-act="remove-admin"]');
  if (!btn) return;

  const email = btn.closest('.admin-row').dataset.email;
  const ok = await confirmDialog({
    title: `remove ${email}?`,
    body: 'they will lose access to the admin app immediately.',
    confirmText: 'remove',
    cancelText: 'cancel',
  });
  if (!ok) return;
  ...
  try {
    await state.db.removeAdminEmail(email);
    await loadAdmins();
    toast(`${email} removed`);
  } catch (err) {
    console.error(err);
    toast(err.message || "couldn't remove that address");
  }
}
...
async function main() {
  ...
  $('#btn-google').addEventListener('click', onGoogleSignIn);
  $('#btn-signout').addEventListener('click', onSignOut);
  $('#btn-signout-denied').addEventListener('click', onSignOut);
  $('#add-admin-form').addEventListener('submit', onAddAdmin);
  $('#admin-rows').addEventListener('click', onAdminRowClick);
  ...
  // Land on the right screen, and follow along if auth changes underneath us
  // (the Google redirect lands back here and resolves a session asynchronously).
  await routeByAuth();
  state.db.onAuthChange(() => {
    const onLoginScreen =
      $('#screen-login').classList.contains('is-active') ||
      $('#screen-denied').classList.contains('is-active');
    if (onLoginScreen) routeByAuth();
  });

  // Keep the queue live while it's on screen.
  watch(state.db, () => {
    if (state.auth.isAdmin && $('#screen-queue').classList.contains('is-active')) refresh();
  }, 8000);
}
